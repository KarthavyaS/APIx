...
export const CitizenFareGuide: React.FC<CitizenFareGuideProps> = ({
  ...
  const [selectedRouteCode, setSelectedRouteCode] = useState<string>(routes[0]?.code || 'DEL-BOM');
  ...
  // Route-specific pricing statistics from real quotes
  const routeQuotes = quotes.filter((q) => q.routeCode === selectedRouteCode);
  const avgTotalFare = routeQuotes.length > 0
    ? Math.round(routeQuotes.reduce((acc, q) => acc + q.totalFare, 0) / routeQuotes.length)
    : (selectedRouteCode === 'DEL-BOM' ? 5450 : selectedRouteCode === 'DEL-BLR' ? 6280 : 4900);
  ...
  const feeBreakdownData = [
    { name: language === 'hi' ? 'मूल किराया (Base Fare)' : 'Base Fare (Airline Revenue)', value: baseFare, color: '#4f46e5' },
    { name: language === 'hi' ? 'जीएसटी कर (5% GST)' : 'GST Tax (5% Government)', value: gstTax, color: '#10b981' },
    { name: language === 'hi' ? 'हवाई अड्डा विकास शुल्क (UDF)' : 'Airport Dev Fee (UDF)', value: udfFee, color: '#f59e0b' },
    { name: language === 'hi' ? 'सुविधा शुल्क (Convenience Fee)' : 'Convenience Fee (OTA/Booking)', value: convenienceFee, color: '#ec4899' },
  ];
  ...
  const faqs = [
    {
      ...
      aEn: 'The National Statistical Office (NSO), Ministry of Statistics and Programme Implementation (MoSPI), tracks retail inflation via the Consumer Price Index (CPI). Since over 90% of domestic flight tickets are booked online with prices fluctuating by 200–400% daily, APIx uses automated ethical data collection across Indian carriers to measure real, transparent price changes that everyday citizens pay.',
      aHi: 'राष्ट्रीय सांख्यिकी कार्यालय (NSO), सांख्यिकी और कार्यक्रम कार्यान्वयन मंत्रालय (MoSPI), उपभोक्ता मूल्य सूचकांक (CPI) के माध्यम से मुद्रास्फीति को मापता है। चूंकि 90% से अधिक हवाई टिकट ऑनलाइन बुक होते हैं और कीमतें प्रतिदिन बदलती हैं, APIx वास्तविक हवाई किराए में बदलाव को पारदर्शी तरीके से ट्रैक करता है।',
    },
    {
      ...
      aEn: 'Indian airlines use dynamic revenue-management algorithms where seats are divided into price tiers. As the departure date nears (e.g. T+1 day vs T+30 days) and seat availability shrinks, remaining seats shift to highest fare buckets. On average, booking 30–45 days ahead saves Indian travellers ₹2,000 to ₹4,500 per ticket.',
      aHi: 'एयरलाइंस डायनेमिक प्राइसिंग प्रणाली का उपयोग करती हैं। जैसे-जैसे यात्रा की तारीख नजदीक आती है (जैसे T+1 दिन बनाम T+30 दिन) और सीटें कम होती हैं, किराया बढ़ जाता है। यात्रा से 30-45 दिन पहले बुकिंग करने पर आमतौर पर ₹2,000 से ₹4,500 तक की बचत होती है।',
    },
    {
      ...
      aEn: 'Your total ticket price comprises four regulated parts: (1) Base Fare kept by the airline, (2) 5% GST on Economy class, (3) User Development Fee (UDF) collected by the airport operator for infrastructure maintenance, and (4) Booking Convenience Fee charged by portals and OTAs.',
      aHi: 'आपके टिकट में 4 मुख्य भाग शामिल हैं: (1) मूल किराया जो एयरलाइन का होता है, (2) 5% जीएसटी कर, (3) हवाई अड्डा विकास शुल्क (UDF) जो एयरपोर्ट के रख-रखाव के लिए होता है, और (4) बुकिंग सुविधा शुल्क (कन्वीनियंस फीस)।',
    },
    {
      ...
      aEn: 'The RBI Monetary Policy Committee (MPC) closely monitors CPI inflation to decide benchmark interest rates (Repo Rate). Airfares are a vital component of the "Transport & Communication" inflation basket. Transparent digital airfare data helps RBI accurately assess transportation cost pressures on Indian households.',
      aHi: 'आरबीआई की मौद्रिक नीति समिति (MPC) रेपो दर तय करने के लिए उपभोक्ता मूल्य सूचकांक (CPI) की निगरानी करती है। परिवहन मुद्रास्फीति के एक महत्वपूर्ण हिस्से के रूप में, यह डेटा नीति निर्माताओं को सटीक निर्णय लेने में मदद करता है।',
    },
    {
      ...
      aEn: 'Yes. APIx is 100% compliant with robots.txt, respects site crawl limits, uses non-intrusive rate-limiting (12–25 req/min), never bypasses CAPTCHAs or security layers, and collects strictly public anonymous tariff metadata with zero personal traveller data.',
      aHi: 'हाँ। APIx पूरी तरह से नैतिक है, वेबसाइटों की robots.txt नीति का सम्मान करता है, और केवल सार्वजनिक मूल्य डेटा एकत्र करता है। इसमें किसी भी नागरिक का व्यक्तिगत डेटा शामिल नहीं होता है।',
    },
  ];
  ...
  const glossary = [
    {
      ...
      simpleDef: 'A national statistical benchmark starting at 100.0 (Base: August 2026). If the index is 108.0, it means domestic flight prices are 8% higher on average than normal.',
    },
    {
    ...
    },
    {
      ...
      simpleDef: 'Base fare is the core charge for the flight seat. Total fare is the final amount paid after adding 5% GST, airport user development fees (UDF), and booking convenience fees.',
    },
    {
  ...
  return (
    <div className="space-y-6 mb-8">
      ...
      <div className="bg-gradient-to-r from-indigo-900 via-slate-900 to-slate-900 text-white rounded-2xl p-6 sm:p-8 border border-indigo-950 shadow-sm relative overflow-hidden">
        <div className="absolute right-0 top-0 w-96 h-96 bg-indigo-500/10 rounded-full blur-3xl pointer-events-none" />
        <div className="relative z-10 max-w-3xl">
          <div className="inline-flex items-center space-x-2 bg-indigo-500/20 border border-indigo-400/30 px-3 py-1 rounded-full text-xs font-medium text-indigo-300 mb-3">
          ...
          <p className="text-sm text-slate-300 leading-relaxed">
            {language === 'hi'
              ? 'सांख्यिकी और कार्यक्रम कार्यान्वयन मंत्रालय (MoSPI) और राष्ट्रीय सांख्यिकी कार्यालय (NSO) द्वारा प्रमाणित सार्वजनिक मूल्य अवलोकन पोर्टल। जानें कि हवाई किराया कैसे तय होता है और सबसे किफायती टिकट कैसे बुक करें।'
              : 'An official Ministry of Statistics & Programme Implementation (MoSPI) public service to help Indian consumers understand flight price movements, fare structures, and optimal booking windows across key domestic routes.'}
          </p>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6 pb-4 border-b border-slate-100">
          ...
          <div className="flex items-center space-x-2">
            ...
            <select
              ...
              className="text-xs font-semibold text-indigo-900 bg-indigo-50/70 border border-indigo-200 rounded-lg px-3 py-1.5 focus:ring-2 focus:ring-indigo-500 focus:outline-none cursor-pointer"
            >
              {routes.map((r) => (
                <option key={r.code} value={r.code}>
                  {r.code} — {r.originCity} to {r.destinationCity}
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="lg:col-span-7 space-y-3">
            ...
            <div className="grid grid-cols-5 gap-2 text-center">
              ...
              <div className="p-3 bg-indigo-50 border border-indigo-200 rounded-xl">
                <span className="text-[10px] font-bold text-indigo-700 block uppercase">30 Days (T+30)</span>
                <span className="text-sm sm:text-base font-extrabold text-indigo-900 font-mono mt-1 block">₹{t30Fare.toLocaleString('en-IN')}</span>
                <span className="text-[9px] text-indigo-600 block mt-1 font-semibold">Save 18%</span>
              </div>

              {/* T+45 */}
              <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-xl ring-2 ring-emerald-400">
                <span className="text-[10px] font-bold text-emerald-700 block uppercase">45 Days (T+45)</span>
                <span className="text-sm sm:text-base font-extrabold text-emerald-900 font-mono mt-1 block">₹{t45Fare.toLocaleString('en-IN')}</span>
            ...
            <div className="p-3.5 bg-blue-50/70 border border-blue-200 rounded-xl flex items-start gap-2.5 text-xs text-blue-900">
              ...
              <div className="leading-relaxed">
                <strong>{language === 'hi' ? 'उपभोक्ता परामर्श:' : 'Consumer Advisory for'} {selectedRoute.originCity} ➔ {selectedRoute.destinationCity}:</strong>{' '}
                {language === 'hi'
          ...
          <div className="lg:col-span-5 bg-slate-50 p-4 rounded-xl border border-slate-200 flex flex-col justify-between">
            <div>
              ...
              <p className="text-[11px] text-slate-500 mb-3">
                Avg Fare ₹{avgTotalFare.toLocaleString('en-IN')} on {selectedRouteCode}
              </p>
            ...
            return (
              ...
              >
                <button
                  ...
                  className="w-full text-left p-4 bg-slate-50/70 hover:bg-slate-100/70 flex justify-between items-center transition cursor-pointer"
                >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {glossary.map((item) => (
            <div
              ...
              className="p-3.5 bg-slate-50/70 rounded-xl border border-slate-200 hover:border-indigo-300 hover:bg-indigo-50/30 transition cursor-pointer flex flex-col justify-between"
            >
              <div>
